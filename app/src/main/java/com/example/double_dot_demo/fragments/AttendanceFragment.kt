...
import com.example.double_dot_demo.models.AttendanceRecord
...
class AttendanceFragment : Fragment() {
    ...
    private fun setupRecyclerView() {
        try {
            adapter = AttendanceAdapter(
                ...
                onAttendanceSwipe = { trainee, sessionId, isPresent ->
                    safeExecute {
                        showAttendanceNoteDialog(trainee, sessionId, isPresent)
                    }
                },
            ...
            // Long-press: show attendance details dialog
            adapter.onAttendanceLongClick = { trainee ->
                safeExecute {
                    showAttendanceDetailsDialog(trainee)
                }
            }
                 ...
                 "Completed" -> {
                     ...
                     filteredList.sortedBy { trainee ->
                         val completedSessions =
                             trainee.attendanceSessions.values.count { record ->
                                 record.isPresent
                             }
    ...
    private fun showAttendanceNoteDialog(
        trainee: Trainee,
        sessionId: String,
        isPresent: Boolean
    ) {
        val editText = EditText(requireContext())
        editText.hint = "Enter note (optional)"

        AlertDialog.Builder(requireContext())
            .setTitle("Attendance Note")
            .setMessage(
                if (isPresent) "Present note" else "Absent reason"
            )
            .setView(editText)
            .setPositiveButton("Save") { _, _ ->
                val note = editText.text.toString().trim()
                updateAttendance(trainee, sessionId, isPresent, note)
            }
            .setNegativeButton("Cancel", null)
            .show()
    }
    private fun showAttendanceDetailsDialog(trainee: Trainee) {

        val sessions = trainee.attendanceSessions
        if (sessions.isEmpty()) {
            Toast.makeText(requireContext(), "No attendance details yet", Toast.LENGTH_SHORT).show()
            return
        }

        val presentLines = mutableListOf<String>()
        val absentLines = mutableListOf<String>()

        sessions
            .toSortedMap(compareBy { it.removePrefix("session_").toIntOrNull() ?: 0 })
            .forEach { (sessionId, record) ->

                val sessionNumber = sessionId.removePrefix("session_")
                val noteText = if (record.note.isBlank()) "No note" else record.note
                val line = "Session $sessionNumber: $noteText"
        ...
        val message = buildString {
            if (presentLines.isNotEmpty()) {
                append("🟢 Present\n")
                presentLines.forEach { append("$it\n") }
                append("\n")
            }
            if (absentLines.isNotEmpty()) {
                append("🔴 Absent\n")
                absentLines.forEach { append("$it\n") }
            }
        }.ifBlank {
        ...
        AlertDialog.Builder(requireContext())
            .setTitle("${trainee.name} - Attendance Notes")
