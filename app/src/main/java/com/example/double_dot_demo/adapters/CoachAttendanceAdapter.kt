...
class CoachAttendanceAdapter(
    ...
    override fun getItemCount(): Int { return try { coaches.size } catch (e: Exception) { 0 } }

    private fun calculateAttendanceStats(coach: Employee): Pair<Int, Int> {
        return try {
            var presentCount = 0
            var absentCount = 0
            coach.attendanceDays.forEach { (_, isPresent) -> if (isPresent) presentCount++ else absentCount++ }
                ...
                override fun onChildDraw(c: android.graphics.Canvas, recyclerView: RecyclerView, viewHolder: RecyclerView.ViewHolder, dX: Float, dY: Float, actionState: Int, isCurrentlyActive: Boolean) {
                    try {
                        if (actionState == ItemTouchHelper.ACTION_STATE_SWIPE) {
                            ...
                            background.color = if (dX > 0) android.graphics.Color.parseColor("#4CAF50") else android.graphics.Color.parseColor("#F44336")
                            background.setBounds(itemView.left, itemView.top, itemView.right, itemView.bottom)
                            background.draw(c)
                        }
                        super.onChildDraw(c, recyclerView, viewHolder, dX, dY, actionState, isCurrentlyActive)
                    } catch (_: Exception) { super.onChildDraw(c, recyclerView, viewHolder, dX, dY, actionState, isCurrentlyActive) }
                }
                override fun onSwiped(viewHolder: RecyclerView.ViewHolder, direction: Int) {
                    try {
                        ...
                        if (position != RecyclerView.NO_POSITION && position < coaches.size) {
                            ...
                            val totalMarkedDays = presentCount + absentCount
                            if (totalMarkedDays >= coach.totalDays) {
                                android.widget.Toast.makeText(recyclerView.context, "Attendance already complete for ${coach.name}", android.widget.Toast.LENGTH_SHORT).show()
                                return
                            }
